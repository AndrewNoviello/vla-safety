...
Keyboard:
  ...
  'f' — mark failure, record N more seconds (see FAILURE_EXTRA), then save (when recording)
...
# ── Config ─────────────────────────────────────────────────────────────────
# Default: <repo>/data/recordings — override with env VLA_RECORD_DATA_ROOT if needed.
_REPO_ROOT = Path(__file__).resolve().parent.parent
DATA_ROOT = Path(os.environ.get("VLA_RECORD_DATA_ROOT", str(_REPO_ROOT / "data" / "recordings")))
ROBOT_NS = "so101real"
# Seconds after 'f' before save (default 5). Override: env VLA_RECORD_FAILURE_EXTRA
FAILURE_EXTRA = max(0.0, float(os.environ.get("VLA_RECORD_FAILURE_EXTRA", "5.0")))
    ...
    def __init__(self, experiment_id: str, task_name: str = TASK_NAME):
        ...
        if not readme.exists():
            readme.write_text(
                f"# {experiment_id}\n\n"
                f"Task: {task_name}\n\n"
                "Generated by scripts/recorder.py\n\n"
                "## Layout\n"
