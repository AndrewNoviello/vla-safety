...
from utils.config_utils import load_config_from_checkpoint
...
class PreTrainedPolicy(nn.Module, HubMixin, abc.ABC):
    ...
    def push_model_to_hub(
        ...
        api = HfApi()
        model_type = getattr(self.config, "type", "pi0") if hasattr(self, "config") and self.config else "pi0"
        resolved_repo_id = (
            repo_id
            or (getattr(self.config, "repo_id", None) if hasattr(self, "config") and self.config else None)
            or (f"{dataset_repo_id.split('/')[0]}/{model_type}-{dataset_repo_id.split('/')[-1]}" if "/" in dataset_repo_id else f"{model_type}-model")
        )
        resolved_private = private
        if hasattr(self, "config") and self.config and getattr(self.config, "private", None) is not None:
            resolved_private = self.config.private

        repo_result = api.create_repo(
            repo_id=resolved_repo_id, private=resolved_private, exist_ok=True
        )

        with TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            saved_path = Path(tmp) / repo_result.repo_id

            if peft_model is not None:
                peft_model.save_pretrained(saved_path)
                if hasattr(self, "config") and self.config and hasattr(self.config, "save_pretrained"):
                    self.config.save_pretrained(saved_path)
            else:
                self.save_pretrained(saved_path)

            resolved_license = license or (getattr(self.config, "license", None) if hasattr(self, "config") and self.config else "apache-2.0")
            resolved_tags = tags or (getattr(self.config, "tags", None) if hasattr(self, "config") and self.config else None)
            card = self.generate_model_card(
                dataset_repo_id, model_type, resolved_license, resolved_tags
            )
    ...
    def generate_model_card(
        ...
        template_path = Path(__file__).parent / "templates" / "lerobot_modelcard_template.md"
    ...
    def wrap_with_peft(
        ...
        pretrained_path = getattr(self.config, "pretrained_path", None) if hasattr(self, "config") and self.config else getattr(self, "pretrained_path", None)
        if pretrained_path:
            self.name_or_path = str(pretrained_path)
