...
def resolve_delta_timestamps(
    cfg: PreTrainedConfig, ds_meta
) -> dict[str, list] | None:
    """Resolve delta_timestamps by reading from the delta_indices properties of a PreTrainedConfig.

    Moved from datasets/factory.py. *ds_meta* can be a ``LeRobotDataset`` (which exposes
    ``.fps`` and ``.features``) or the former ``LeRobotDatasetMetadata``.
    """
