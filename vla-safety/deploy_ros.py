...
A single ROS2 node that connects to two praxis nodes already running:

  praxis (separate repo, already running)
  ...
  vla-safety (this script)
  └── SafeDeploymentNode   → subscribes to so101real/joint_state     ← from praxis
                           → subscribes to so101real/camera/image    ← from praxis
                           → publishes  so101real/joint_command      → to praxis

Data flow:

  praxis/webcam ──→ so101real/camera/image ──┐
                                             ├──→ PI0 (base policy) → proposed action ──┐
  praxis/robot  ──→ so101real/joint_state  ──┘                                          │
                                                                                        ▼
                                                                        ┌──────────────────────────┐
                                                                        │ LatentSafetyFilter       │
                                                                        │ V(ẑ_{t+1}) > ε ?        │
                                                                        ...
                                                                        └──────────────────────────┘
                                                                                        │
                                                                                        ▼
  praxis/robot  ←── so101real/joint_command ←────────────────────────────────────────────┘

Prerequisites (run in the praxis repo first):
    cd /path/to/praxis
    python -m praxis.ros.scripts.robot_node_launcher robot=so101real &
    python praxis/ros/examples/webcam.py &

Then run this script from the vla-safety repo:
    python deploy_ros.py \
    ...
    python deploy_ros.py --disable-safety --stats-path stats.json
"""
...
class SafeDeploymentNode(Node):
    ...
    def __init__(
        ...
        self.get_logger().info(
            ...
            f"Waiting for praxis topics ..."
        )

    # ------------------------------------------------------------------
    # ROS callbacks (receive data from praxis)
    # ------------------------------------------------------------------

    def _cb_joint_state(self, msg: JointState):
        """Receive joint positions from praxis/robot_node_launcher."""
        positions = np.array(msg.position[:PROPRIO_DIM], dtype=np.float32)
        with self._lock:
            self._latest_proprio = positions

    def _cb_camera(self, msg: Image):
        """Receive camera frames from praxis/webcam.py."""
    ...
    def _control_tick(self):
        ...
        if image is None or proprio is None:
            return  # Praxis nodes not publishing yet
        ...
        # --- Publish to praxis/robot_node_launcher ---
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            ...
            "Requires praxis nodes to be running first:\n"
            "  cd /path/to/praxis\n"
            "  python -m praxis.ros.scripts.robot_node_launcher robot=so101real &\n"
            "  python praxis/ros/examples/webcam.py &\n"
        ),
    ...
    p.add_argument("--stats-path", default="stats.json")
