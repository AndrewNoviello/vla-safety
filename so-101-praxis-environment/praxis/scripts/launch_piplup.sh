...
# Launch script for Piplup teleoperation system
...
echo -e "${GREEN}Starting Piplup teleoperation system...${NC}"
...
cleanup() {
    ...
    pkill -TERM -f "robot_node_launcher.py.*robot=piplup" 2>/dev/null
    pkill -TERM -f "piplup-teleop.py" 2>/dev/null
    sleep 0.5
    pkill -9 -f "robot_node_launcher.py.*robot=piplup" 2>/dev/null
