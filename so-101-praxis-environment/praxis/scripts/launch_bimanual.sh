...
# Launch Piplup teleop node
echo -e "${BLUE}Launching teleop node for Piplup...${NC}"
python3 "$SCRIPT_DIR/../ros/examples/piplup-teleop.py" &
PIPLUP_TELEOP_PID=$!

# Launch Rowlet teleop node
...
echo -e "${YELLOW}Piplup Teleop node PID: $PIPLUP_TELEOP_PID${NC}"
echo -e "${YELLOW}Rowlet Robot node PID: $ROWLET_ROBOT_PID${NC}"
echo -e "${YELLOW}Rowlet Teleop node PID: $ROWLET_TELEOP_PID${NC}"
...
wait $PIPLUP_ROBOT_PID $PIPLUP_TELEOP_PID $ROWLET_ROBOT_PID $ROWLET_TELEOP_PID $SENSORR_SENSOR_PID $SENSORL_SENSOR_PID
